...
var ToggleMixin = {
   ...
   */
  isVisible: function () {
    return !this.el.parentNode.classList.contains("sidebar-collapsed");
  },
  ...
  _viewController: function ({ visible, animated, delayed }) {
    // TODO: ANIMATE THE COLLAPSE/EXPAND. FOR NOW, JUST HIDE/SHOW BY TOGGLING A CLASS.
    // let flags = {
    //   visible: visible,
    ...
    this.el.parentNode.classList.toggle("sidebar-collapsed", !visible);

    if (visible) {
      this.button.classList.remove("pane-collapsed");
      this.button.setAttribute("title", this._collapseString);
    }
    else {
      this.button.classList.add("pane-collapsed");
      this.button.setAttribute("title", this._expandString);
    }
  },
